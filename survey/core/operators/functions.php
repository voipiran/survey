...
function user($agent_number)
{
    include '../config/db.php';
    $user = $conn->query("SELECT * FROM survey WHERE agent_number = $agent_number");
...
function create_line()
{
    ...
    $year         = $_GET['year'];
    $month        = $_GET['month'];
    $agent_number = $_GET['agent_number'];
    $m = $month;
    $m++;
    if ($m == 13) {
        $m = 01;
...
function chart()
{
    ...
    if ($max == 0) {
        ...
        return json_encode($bar_chart_data);
    }
